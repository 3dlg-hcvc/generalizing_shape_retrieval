...
def get_obj_lfd(obj_id, num_views=200):
    lfds = []
    model_name = obj_id.split('/')[-1]
    for view_idx in range(num_views):
        mask_path = mask_path_template.format(obj_id=obj_id, model_name=model_name, view_idx=view_idx)
...
if __name__ == '__main__':
    data_dir = "/path_to_pix3d"
    mask_dir = f"{data_dir}/lfd_200"
    mask_path_template = data_dir + "/lfd_200/{obj_id}/{model_name}-{view_idx}.color.encoded.png"
    obj_ids = [n.strip() for n in open("./data/pix3d/pix3d_model_path_filtered.txt")]
    num_obj = len(obj_ids)
